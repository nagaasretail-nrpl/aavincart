...
import {
  ...
  CheckCircle, Activity, Ban, User, Phone, CreditCard,
} from "lucide-react";
...
const VEHICLE_TYPES = ["van", "truck", "tempo", "bike", "auto"];
...
export default function MerchantDeliveryVehicles() {
  ...
  const drivers = vehicles.filter((v) => v.driverName).map((v) => ({
    id: v.id,
    name: v.driverName,
    phone: v.driverPhone,
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg sm:text-xl font-bold">Vehicles & Fleet</h1>
            <p className="text-xs sm:text-sm text-muted-foreground">Vehicle & Driver Master Data</p>
          </div>
        </div>
        ...
        <Tabs defaultValue="vehicles">
          ...
          <TabsContent value="drivers">
                ...
                <CardContent className="p-2 sm:p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xl sm:text-2xl font-bold">{drivers.filter(d => d.status === "available" || d.status === "on_delivery").length}</p>
                    </div>
                    <CheckCircle className="h-6 w-6 sm:h-8 sm:w-8 text-green-200" />
                  </div>
                </CardContent>
              ...
              <Card className="bg-gradient-to-br from-indigo-500 to-indigo-600 text-white">
                <CardContent className="p-2 sm:p-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-indigo-100 text-xs">On Delivery</p>
                      <p className="text-xl sm:text-2xl font-bold">{drivers.filter(d => d.status === "on_delivery").length}</p>
                    </div>
                    <Activity className="h-6 w-6 sm:h-8 sm:w-8 text-indigo-200" />
                  </div>
                </CardContent>
            ...
            <Card>
              <CardContent className="p-0">
                {vehiclesLoading ? (
                  <div className="p-12 text-center text-muted-foreground">Loading drivers...</div>
                ) : drivers.length === 0 ? (
                  <div className="p-12 text-center">
                    <User className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
                    <h3 className="text-lg font-medium mb-2">No Drivers</h3>
                    <p className="text-muted-foreground">Add drivers through Vehicle Master by assigning a driver to a vehicle.</p>
                  </div>
                ) : (
                  <>
                    <div className="hidden md:block overflow-x-auto">
                        ...
                        <TableBody>
                          {drivers.map((d) => (
                            <TableRow key={d.id}>
                              ...
                              <TableCell>{vehicleStatusBadge(d.status)}</TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
                      </Table>
                        ...
                        <Card key={d.id} className="border">
                          <CardContent className="p-3 space-y-2">
                            <div className="flex items-center justify-between">
                              ...
                              {vehicleStatusBadge(d.status)}
                            </div>
                            <div className="grid grid-cols-2 gap-1 text-xs text-muted-foreground">
                              <span className="flex items-center gap-1"><Phone className="h-3 w-3" /> {d.phone || "N/A"}</span>
                              <span className="flex items-center gap-1"><CreditCard className="h-3 w-3" /> {d.license || "N/A"}</span>
                              <span>Vehicle: {d.vehicleNumber}</span>
                              <span className="capitalize">Type: {d.vehicleType}</span>
                            </div>
                          </CardContent>
                        </Card>
                      ))}
