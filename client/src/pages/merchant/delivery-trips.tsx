...
export default function MerchantDeliveryTrips() {
  const { merchantId, staffSession, user } = useMerchantContext();
  const { toast } = useToast();

  const canCreateTrips = staffSession?.designationId === 'transport_manager' || user?.role === 'admin';
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-muted-foreground">
                {canCreateTrips ? "Plan trips step-by-step and manage existing ones" : "Monitor and view existing trip sheets"}
              </p>
            </div>
          </div>
