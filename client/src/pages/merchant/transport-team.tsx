...
export default function MerchantTransportTeam() {
  ...
  const { data: staffList = [], isLoading: staffLoading } = useQuery<StaffMember[]>({
    ...
    queryFn: async () => {
      const res = await fetch("/api/staff?department=delivery", { credentials: "include" });
      if (!res.ok) return [];
      return res.json();
    },
  });
  ...
  const createManagerMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setManagerForm({ name: "", phone: "", employeeId: "", username: "", email: "", password: "", segments: [] });
    },
  ...
  const createDriverMutation = useMutation({
    mutationFn: async (data: typeof driverForm) => {
      ...
      if (data.vehicleId && data.vehicleId !== "none" && result.id) {
    ...
    onSuccess: () => {
      ...
      setDriverForm({ name: "", email: "", phone: "", password: "", assignedSegment: "", vehicleId: "" });
    },
    onError: (e: any) => toast({ title: "Error", description: e.message, variant: "destructive" }),
  });
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="managers" className="space-y-4">
            ...
            <div className="flex justify-end mb-4">
              <Button onClick={() => setShowAddManagerDialog(true)} className="bg-purple-600 hover:bg-purple-700">
          ...
          <TabsContent value="drivers" className="space-y-4">
            ...
            <div className="flex justify-end mb-4">
              <Button onClick={() => { setDriverForm({ name: "", email: "", phone: "", password: "", assignedSegment: "", vehicleId: "" }); setShowAddDriverDialog(true); }} className="bg-purple-600 hover:bg-purple-700">
                <Plus className="h-4 w-4 mr-2" />
                Add Driver
              </Button>
            </div>
              ...
              <CardContent className="p-0">
                {vehiclesLoading ? (
                  ...
                  <div className="p-12 text-center">
                    ...
                    <p className="text-muted-foreground">Add vehicles from the Vehicles & Fleet page.</p>
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <Table>
          ...
          <DialogContent className="w-[95vw] sm:max-w-lg">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                Add Transport Manager (Staff)
              </DialogTitle>
              <p className="text-sm text-muted-foreground mt-1">
                This creates a <span className="font-medium">staff member</span> with transport management permissions, not a driver.
              </p>
            </DialogHeader>
            <div className="space-y-4">
              <div className="rounded-lg border bg-purple-50 dark:bg-purple-950/30 p-3 space-y-2">
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Department</p>
                    <p className="text-sm font-semibold text-purple-700 dark:text-purple-300">Transport</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Designation</p>
                    <p className="text-sm font-semibold text-purple-700 dark:text-purple-300">Transport Manager</p>
                  </div>
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-medium">Access Tier</p>
                    <p className="text-sm font-semibold text-purple-700 dark:text-purple-300">Union Level</p>
                  </div>
                </div>
                <p className="text-[11px] text-muted-foreground">
                  Can access Delivery Dashboard, Trip Planning, Live Tracking, and Vehicles & Fleet.
                </p>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mb-2">Select which product segments this manager oversees. Leave empty for all segments.</p>
            ...
            <DialogFooter>
              ...
              >
                {createManagerMutation.isPending ? "Creating..." : "Create Transport Manager"}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
