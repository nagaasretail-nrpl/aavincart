...
import { ArrowLeft, Download, Search, LogIn } from "lucide-react";
...
export default function SignedInUsers() {
  ...
  const { data: users, isLoading } = useQuery<SignedInUser[]>({
    queryKey: [`/api/admin/user-metrics/signed-in?range=${range}&search=${encodeURIComponent(search)}`],
  });

  async function handleExport() {
    if (!users || users.length === 0) return;
    try {
      const headers = ["User ID", "Name", "Phone/Email", "Role", "Union", "Last Sign-in Time"];
      ...
      const buf = await buildXlsxBuffer([{ name: "Signed-in Users", rows: [headers, ...rows] }]);
      ...
      a.href = url;
      a.download = `Signed_In_Users_${range}_${new Date().toISOString().slice(0, 10)}.xlsx`;
      a.click();
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <h1 className="text-xl font-bold text-gray-900 dark:text-white">Signed-in Users</h1>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b bg-gray-50 dark:bg-gray-800/50">
                    ...
                    <th className="text-left p-3 font-medium text-gray-600 dark:text-gray-400">Last Sign-in</th>
                  </tr>
                </thead>
                <tbody>
                  {isLoading ? (
                    ...
                    <tr>
                      <td colSpan={5} className="p-8 text-center text-gray-400">
                        No signed-in users found for this period
                      </td>
                    </tr>
                  )}
                </tbody>
