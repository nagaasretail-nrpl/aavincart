...
import { Package, Truck, Users, Sun, Moon, Search, ExternalLink, Clock, CheckCircle, AlertTriangle, RefreshCw, Route } from "lucide-react";
...
export default function AdminRegularDelivery() {
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <a href="/pwa/transport" target="_blank" rel="noopener noreferrer">
            <Button variant="outline" size="sm" className="gap-2">
              <ExternalLink className="h-4 w-4" /> Transport Manager
            </Button>
          </a>
        </div>
        ...
        <Tabs value={activeTab} onValueChange={(val) => {
          setActiveTab(val);
          window.history.replaceState(null, '', `/admin/regular-delivery?tab=${val}`);
        }}>
          ...
          <TabsContent value="dashboard">
            <RegularDashboard merchantId={merchantId} />
          </TabsContent>
          <TabsContent value="orders">
            <OrderDeliveriesPanel merchantId={merchantId} onAssign={() => { setActiveTab("trips"); window.history.replaceState(null, '', '/admin/regular-delivery?tab=trips'); }} />
          </TabsContent>
          <TabsContent value="trips">
            <TripPlanningWizard merchantId={merchantId} isAdmin={isAdmin} canCreateTrips={true} deliveryType="regular" />
          </TabsContent>
          <TabsContent value="fleet">
            <FleetVehiclesPanel merchantId={merchantId} isAdmin={isAdmin} />
          </TabsContent>
          <TabsContent value="drivers">
            <DriversPanel merchantId={merchantId} isAdmin={isAdmin} />
          </TabsContent>
        </Tabs>
      </div>
    </AdminLayout>
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2 mb-2"><Clock className="h-4 w-4 text-orange-500" /><span className="text-sm font-medium">Pending</span></div>
            <p className="text-2xl font-bold">{stats?.pendingValidation || 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2 mb-2"><CheckCircle className="h-4 w-4 text-green-500" /><span className="text-sm font-medium">Ready for Trip</span></div>
            <p className="text-2xl font-bold">{stats?.readyForTrip || 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2 mb-2"><Truck className="h-4 w-4 text-blue-500" /><span className="text-sm font-medium">In Transit</span></div>
            <p className="text-2xl font-bold">{stats?.inTransit || 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-4">
            <div className="flex items-center gap-2 mb-2"><CheckCircle className="h-4 w-4 text-emerald-500" /><span className="text-sm font-medium">Delivered</span></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: "Fresh Milk", jobs: fmJobs, color: "blue" },
          { label: "Products", jobs: dpJobs, color: "purple" },
          { label: "Ice Cream", jobs: icJobs, color: "pink" },
        ].map(seg => (
          <Card key={seg.label}>
            <CardHeader className="pb-2">
              <CardTitle className="text-sm">{seg.label}</CardTitle>
            </CardHeader>
            <CardContent>
...
function OrderDeliveriesPanel({ merchantId, onAssign }: { merchantId: string; onAssign?: () => void }) {
  ...
  const handleRefresh = () => {
    queryClient.invalidateQueries({ queryKey: ["/api/delivery-jobs", merchantId, "regular"] });
  };
