...
export default function WsdLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/agent/login", {
        ...
        credentials: "include",
        body: JSON.stringify({ agentCode: agentCode.trim().toUpperCase(), password }),
      });
    ...
    <div className="min-h-screen lg:h-screen flex flex-col lg:flex-row overflow-hidden">
      <div className="hidden lg:flex lg:w-[45%] flex-shrink-0 relative">
        ...
        <div className="absolute bottom-8 left-8 right-8 text-white">
          <h2 className="text-3xl font-bold mb-2">Wholesale Dealer / MPCS Portal</h2>
          <p className="text-lg opacity-90">Manage your wholesale dairy distribution</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-[55%] flex items-center justify-center px-3 sm:px-4 py-6 sm:py-8 bg-gray-50 overflow-y-auto">
        <div className="w-full max-w-lg">
          ...
          <p className="text-center text-gray-500 mb-4 sm:mb-5 text-sm">
            Wholesale Dealer & Milk Producers' Cooperative Societies
          </p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4 sm:p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">WSD Code / Mobile Number</label>
                <Input
                  type="text"
                  placeholder="e.g., WSD001, MPCS023, or 9876543210"
                  value={agentCode}
                  onChange={(e) => setAgentCode(e.target.value)}
                  className="h-11 border-gray-300"
                  autoComplete="username"
                />
                <p className="text-xs text-gray-500 mt-1">Enter your WSD/MPCS code or registered mobile number</p>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Password (Mobile Number)</label>
                ...
                <p className="text-xs text-gray-500 mt-1">Use your registered mobile number as password</p>
              </div>

              <Button
                type="submit"
                className="w-full h-11 bg-gradient-to-r from-purple-500 to-purple-600 hover:opacity-90 text-white font-medium"
                disabled={isLoading}
              >
            ...
            <div className="mt-4 text-center text-sm text-gray-600">
              Having trouble logging in? Contact your District Union Office
            </div>
          </div>
          ...
          <div className="mt-4 text-center">
            <div className="flex items-center justify-center gap-3 text-xs text-gray-400 flex-wrap">
              ...
              <span>|</span>
              <Link href="/fmd/login" className="hover:text-gray-600 hover:underline">FMD Login</Link>
              <span>|</span>
